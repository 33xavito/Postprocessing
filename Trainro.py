...
def train(epoch, trainloader, model, optimizer, criterion, args, device):
    ...
    for batch_idx, (inputs, targets, scale_mean, scale_std) in tqdm(enumerate(trainloader), desc=f'Epoch {epoch}: ',
        ...
        mu = output[...,0].mean(dim=1).view(batch_size, 20, 276) #20
        mu = mu* scale_std + scale_mean
        sigma=output[...,0].std(dim=1).view(batch_size, 20, 276) #20
        ...
        if args.loss == 'CRPS':
            ...
            ensemble = output.squeeze(-1).view(batch_size,11,20, 276) 
            ensemble = ensemble* scale_std + scale_mean 
           # ensemble = torch.clamp(ensemble, min=0.0) #no negative values for windspeed
            ensemble= ensemble.view(batch_size, 11,20, 23,12)
            
            kernel_target=targets.view(batch_size, 20, 23,12)
...
def test(epoch, testloader, model, criterion, args, device):
    ...
    with torch.no_grad():
        for batch_idx, (#dates, 
            ...
            if args.loss == 'CRPS':
                ...
                ensemble = output.squeeze(-1).view(batch_size,11,20, 276) 
                ensemble = ensemble* scale_std + scale_mean 
                ensemble= ensemble.view(batch_size, 11,20, 276)
                kernel_target=targets.view(batch_size, 20, 276)
                loss=criterion(ensemble, kernel_target)
    ...
    if crps_loss < best_crps:
        ...
        #f"_lambda{args.lambda_reg}k{args.k_reg}_{args.decay_type}"
        f"lr{args.lr}epochs{args.epochs}b{args.batch_size}heads{args.nheads}mlt{args.mlp_mult}Stack{args.num_blocks}{args.projection_channels}"
...
log_file_path = f'./results/Transformerweights/reducedororgraphy/{args.target_var}/training_log.txt'  
