...
class EUPPFullEnsembleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ds_eupp = xr.open_dataset(self.eupp_files[idx])
            ...
            min_lat, max_lat = 48.5, 45.75
            min_lon, max_lon = 5, 10.5
            ...
            inputs = torch.zeros((len(self.variables), self.num_ensemble, len_TD,23,12)) #(3,51,32,33) 
            for k in range(len(self.variables)): #len is 1
                ...
                if variable == 'oro': # Check if variable is orography
                    ...
                    #print(variable, ds_eupp[variable].shape)
                    values = torch.reshape(torch.as_tensor(ds_eupp[variable].to_numpy()[:self.num_ensemble,:]), (self.num_ensemble,len_TD,23,12)) #(11,32,33)
                if self.normalized:
